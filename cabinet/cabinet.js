...
function renderRefs(){
  ...
  if(tiers&&Array.isArray(r.tiers)&&r.tiers.length){
    tiers.innerHTML=r.tiers.map(t=>`<div class="tier"><b>${safeText(t.display_name)}</b><p>${Number(t.min_qualified_referrals)===0?'Starting tier.':`${Number(t.min_qualified_referrals).toLocaleString()} active referrals.`}</p><span>${(Number(t.commission_bps)/100).toFixed(0)}%</span></div>`).join('');
  }
  ...
  if(joined){
    ...
    $('#affRateText').textContent=`${(Number(r.commission_bps||0)/100).toFixed(0)}% effective rate · ${r.rate_source==='custom'?'bespoke rate':'public tier'} · lifetime attribution.${r.commissions_enabled?' Commission is live.':' Commission starts when the programme is activated.'}`;
  }

  const recent=$('#recentRefs');
  if(recent){
    const rows=Array.isArray(r.recent_referrals)?r.recent_referrals:[];
    recent.innerHTML=rows.length?`<div class="refrow head"><span>Referral</span><span>Status</span><span>Fees generated</span><span>Commission earned</span></div>`+rows.map(x=>`<div class="refrow"><b>${safeText(x.player_ref)}</b><span class="${x.qualified?'good':''}">${x.qualified?'Active':'Signed up'}</span><span>${money(x.platform_fee_minor||0)}</span><span>${money(x.commission_minor||0)}</span></div>`).join(''):'<div class="empty">No referrals yet. Your link is ready when you are.</div>';
  }

  const note=$('.cashnote');
  if(note)note.textContent=r.commissions_enabled
    ? `You earn ${(Number(r.commission_bps||0)/100).toFixed(0)}% of eligible TH3FLOW platform fees generated by your lifetime referrals.`
    : 'Referral attribution is live. Commission accrual begins when the programme is activated.';

  const payout=$('#requestPayout');
  if(payout){const enough=Number(r.payable_balance_minor||0)>=Number(r.min_payout_minor||0)&&Number(r.min_payout_minor||0)>0;payout.disabled=!data.session;payout.textContent=enough?'Request payout':`Minimum ${money(r.min_payout_minor||0)}`;payout.title=enough?'Request affiliate payout':`Minimum payout is ${money(r.min_payout_minor||0)} · available ${money(r.payable_balance_minor||0)}`;}const pm=$('#payoutMinimum');if(pm)pm.textContent=`Minimum ${money(r.min_payout_minor||0)} · available ${money(r.payable_balance_minor||0)}`;
}
function render(){
...
$('#shareRef').onclick=async()=>{
  ...
  if(!data.referral?.code)return toast('Sign in first');
  try{
    if(navigator.share){await navigator.share({title:'TH3FLOW',text:'TH3FLOW — Higher or Lower.',url:v});return}
    await navigator.clipboard.writeText(v);toast('Referral link copied');
  }catch(e){if(e?.name!=='AbortError')toast('Share unavailable')}
};
$('#whatsappRef').onclick=()=>{
  const v=$('#refLink').value;
  if(!data.referral?.code)return toast('Sign in first');
  const msg=`TH3FLOW — Higher or Lower.
${v}`;
